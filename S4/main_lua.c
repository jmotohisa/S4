...
static int S4L_NewSimulation(lua_State *L){
	if(!lua_istable(L, 1)){
		return luaL_error(L, "expected named arguments");
	}
	...
	if(lua_isnumber(L, -1)){
		...
		for(int i = 0; i < 2; ++i){
			...
			lua_gettable(L, -2);
			if(!lua_istable(L, -1)){
				return luaL_error(L, "lattice must be a list of 2 vectors");
			}
			for(int j = 0; j < 2; ++j){
				lua_pushinteger(L, j+1);
				lua_gettable(L, -2);
				if(!lua_isnumber(L, -1)){
					return luaL_error(L, "lattice vectors must be numeric");
				}
				Lr[2*i+j] = lua_tonumber(L, -1);
				lua_pop(L, 1);
			}
			lua_pop(L, 1);
		}
	}else{
		return luaL_error(L, "must specify lattice as either a positive real number or a list of 2 vectors");
	}
	lua_pop(L, 1);

	lua_getfield(L, 1, "basis_size");
	if(lua_isnumber(L, -1)){
		nG = lua_tointeger(L, -1);
		if(nG < 1){
			return luaL_error(L, "basis_size must be positive");
		}
	}else if(lua_isnil(L, -1)){
		lua_pop(L, 1);
		lua_getfield(L, 1, "basis_set");
		if(lua_istable(L, -1)){
			lua_len(L, -1);
			nG = lua_tointeger(L, -1);
			lua_pop(L, 1);
			...
			for(int i = 0; i < nG; ++i){
				...
				lua_gettable(L, -2);
				if(!lua_istable(L, -1)){
					return luaL_error(L, "basis_set must contain vectors as tables of length 2");
				}
				for(int j = 0; j < 2; ++j){
					lua_pushinteger(L, j+1);
					lua_gettable(L, -2);
					if(!lua_isnumber(L, -1)){
						return luaL_error(L, "basis_set vectors must be integral");
					}
			...
			return luaL_error(L, "must specify basis_set as a list of vectors");
		}
	}else{
		return luaL_error(L, "must specify basis_size as a positive integer");
	}
	lua_pop(L, 1);
